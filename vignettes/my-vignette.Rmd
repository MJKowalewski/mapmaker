...
If the map crosses the dateline, the longitude West values are positive > 180 rather than negative (-180 - 0).

```{r creating a map (bearing strait), echo=TRUE, fig.dim = c(5, 5), fig.align='center'}
mapmaker(bearing.straits)
```

Also, custom dimensions can be defined to customize map aspect ratio (e.g., mapmaker(bearing.straits, pdf = TRUE, custom.dim = c(6,5)).
